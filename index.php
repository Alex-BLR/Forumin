...
if (isset($_REQUEST['add'])) {
	if (strtolower($_REQUEST['secpic']) !=$_SESSION['secpic']) {
		...
		exit("<html><head><meta http-equiv='Content-Type' content='text/html; charset=windows-1251'></head><body><div style='display: flex; flex-direction: column; align-items: center; justify-content: center; margin-top: 100px; font-size: 13px; font-weight: bold; font-family: tahoma; min-height: 50vh;'><fieldset style='width: 400px; border: #333 1px solid; text-align: center;'><legend style='color: red;'>ВНИМАНИЕ</legend><p>Неверно введен защитный код!</p></fieldset><a href='javascript:history.back()' style='text-decoration: none; margin-top: 30px;'>&#9668; назад</a></div>");
	}
}
...
if ($antiham == 1) {
	...
	if ($bad_ips_raw) {
		...
		if (in_array($ip, $bad_ips)) {
			die("<div style='display: flex; flex-direction: column; align-items: center; justify-content: center; margin-top: 100px; font-size: 13px; font-weight: bold; font-family: tahoma; min-height: 50vh;'><fieldset style='width: 400px; border: #333 1px solid; text-align: center;'><legend style='color: red;'>Вы заблокированы!</legend><p>Администратор запретил вам пользоваться форумом!</p></fieldset></div>");
		}
	}
}
...
if (isset($_GET['mode']) and $_GET['mode']=="reg")
{
	if (isset($_POST['name']) && isset($_POST['mail']) && isset($_POST['passreg']))
	{
		...
		/////////////// Записываем строчку в файл со статистикой
...
if (isset($name) && isset($cname) && isset($msg) && isset($email) && isset($_POST['forumid']) && isset($_POST['action']) && $_POST['action']=="answer")
{
	if (strlen($msg)>2 && strlen($msg)<$maxmsg)
	{
		...
		show_error("Сообщение не должно быть пустым или менее 2-х символов, не должно превышать $maxmsg символов!", "javascript:history.back(1)");
	}
}
...
<br><p align=center><a href=\"javascript:history.back(1)\">&#9668; назад</a></p></body></html>";
	...
	} else echo "<div align=center><br><br><br><br><br><fieldset style='border: #555 1px solid; width:350px'><legend align='center'><font size=2 face=tahoma color=red><b>Внимание!</b></font></legend>
			<br><center><font size=2 face=tahoma><b>Вы не можете редактировать это сообщение, установленное для этого время истекло!</b></font></center>
...
<input type=\"checkbox\" name=\"check1\" onClick=\"checkedBox(this.form)\"></td><td> &nbsp; я не бот</td></tr></table></td></tr><tr><td colspan=2><div id=other align=center><input type=reset class=fbutton value='УДАЛИТЬ'> &nbsp; &nbsp; <input type=button class=fbutton value='ПРОСМОТР' onClick='seeTextArea(this.form)'> &nbsp; &nbsp; <input type=submit class=fbutton value='ОТПРАВИТЬ' disabled=disabled></div></td></tr></form></td></table>";
				...
				for($a=0; $a<count($lines); $a++)
				{
					...
					if ($forumid==$dtt[2]) {
						if ($dtt[11]=="0") exit("<div align=center style='color:red;font-family:verdana;font-size:12px;font-weight:bold'>Тема закрыта!<br><br><a href='index.php'>вернуться назад</a><br></div>"); else print $formreg;
					}
				}
			}
		} else {
